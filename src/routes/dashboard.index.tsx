  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle={t("dash_welcome_sub")}
      />

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label={t("dash_available_balance")} value={formatDZD(availableBalance)} icon={Wallet} tone="brand" />
        <StatCard label={t("dash_pending_balance")} value={formatDZD(pendingBalance)} icon={Clock} tone="warning" />
        <StatCard label={t("dash_total_earnings")} value={formatDZD(totalEarnings)} icon={TrendingUp} tone="success" />
        <StatCard label={t("dash_delivered_orders")} value={delivered.length.toString()} icon={Package} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <ChartCard title={t("dash_monthly_earnings")} subtitle={t("dash_last_7_months")} className="lg:col-span-3">
          {earningsChart.length === 0 ? (
            <div className="h-[280px] flex items-center justify-center text-muted-foreground text-sm">{t("dash_no_data")}</div>
          ) : (
        ...
        <ChartCard title={t("dash_delivered_volume")} subtitle={t("dash_volume_month")} className="lg:col-span-2">
          {earningsChart.length === 0 ? (
            <div className="h-[280px] flex items-center justify-center text-muted-foreground text-sm">{t("dash_no_data")}</div>
          ) : (
      ...
      <div className="rounded-2xl border bg-card">
        <div className="flex items-center justify-between p-5">
          <div>
            <h2 className="font-semibold text-lg" dir="auto">{t("dash_latest_orders")}</h2>
            <p className="text-sm text-muted-foreground">{t("dash_latest_orders_sub")}</p>
          </div>
          <Button asChild variant="ghost" size="sm"><Link to="/dashboard/orders">{t("dash_view_all")} <ArrowRight className="ml-1 h-4 w-4" /></Link></Button>
        </div>
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {ordersLoading ? (
                <TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground animate-pulse">{t("dash_loading")}</TableCell></TableRow>
              ) : recent.length === 0 ? (
                <TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">{t("dash_no_orders")}</TableCell></TableRow>
              ) : recent.map((o) => (
                <TableRow key={o.id}>
...
function ChartCard({ title, subtitle, children, className = "" }: { title: string; subtitle: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded-2xl border bg-card p-5 ${className}`}>
      <div className="mb-4"><h2 className="font-semibold" dir="auto">{title}</h2><p className="text-sm text-muted-foreground">{subtitle}</p></div>
      {children}
    </div>
  );
}
